...
// Version: 1.2-hf-ingest-receipt-hedera-network-v1 | 2026-09-09
// Purpose:
//   Deterministic receipt builder for generic ingest flows.
// Notes:
//   - Pure, side-effect free.
//   - Receipt ID hashes the receipt body excluding receipt_id.
//   - Local receipts remain network-independent.
//   - Final register_and_anchor receipts may commit Hedera network identity.
// ============================================================================
...
function normalizeHederaNetwork(value: unknown): HederaNetwork | null {
  ...
  if (network !== "testnet" && network !== "mainnet") {
    throw new IngestError("hedera_network_invalid", {
      code: "SCHEMA_INVALID",
      statusCode: 400,
    });
  }
...
function pickProjectedAnchor(src: unknown): Record<string, unknown> | undefined {
    ...
    ...(publish
      ? {
          publish: stripUndefined({
            hedera_network: publish.hedera_network ?? publish.network ?? undefined,
            ...
            transaction_id:
              publish.transaction_id ?? publish.hcs_transaction_id ?? undefined,
    ...
    ...(certificate
      ? {
          certificate: stripUndefined({
            ...(certificate.attempted !== undefined
              ? { attempted: Boolean(certificate.attempted) }
              : {}),
            ...(certificate.skipped !== undefined
              ? { skipped: Boolean(certificate.skipped) }
              : {}),
            ...(certificate.issued !== undefined
              ? { issued: Boolean(certificate.issued) }
              : {}),
            ...(certificate.deduped !== undefined
              ? { deduped: Boolean(certificate.deduped) }
              : {}),
            ...(certificate.requested !== undefined
              ? { requested: Boolean(certificate.requested) }
              : {}),
  ...
  if (
    isPlainObject(projected.publish) &&
    Object.keys(projected.publish).length === 0
  ) {
    delete (projected as any).publish;
  }

  if (
    isPlainObject(projected.certificate) &&
    Object.keys(projected.certificate).length === 0
  ) {
...
export function buildIngestReceiptV1(opts: {
  ...
  const hedera_network = normalizeHederaNetwork(opts.hedera_network);

  if (hedera_network && opts.mode !== "register_and_anchor") {
    throw new IngestError("receipt_local_hedera_network_forbidden", {
      code: "SCHEMA_INVALID",
      statusCode: 400,
    });
  }
  ...
  const receipt_anchor = pickProjectedAnchor(
    (opts.core as any)?.receipt_anchor
  );
